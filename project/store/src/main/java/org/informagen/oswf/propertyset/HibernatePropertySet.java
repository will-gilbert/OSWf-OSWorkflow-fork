...
import org.informagen.oswf.propertyset.hibernate.HibernatePropertySetDAO;
...
public class HibernatePropertySet extends AbstractPropertySet {
    ...
    protected static Logger logger = LoggerFactory.getLogger(HibernatePropertySet.class);
    ...
    public HibernatePropertySet(Map<String,String> configurationProperties, Map<String,Object> args) {
    ...
    public Collection<String> getKeys(String itemKey, Type type) throws PropertySetException {
        return configProvider.getPropertySetDAO().getKeys(piid, itemKey, type);
    }

    public Type getType(String itemKey) throws PropertySetException {
        
        if(exists(itemKey) == false)
            throw new PropertySetException("itemKey: " + itemKey + " does not exist.");
    ...
    public void remove(String itemKey) throws PropertySetException {
    ...
    protected void setImpl(Type type, String itemKey, Object value) throws PropertySetException {

        HibernatePropertySetItem item = configProvider.getPropertySetDAO().findByKey(piid, itemKey);

        if (item == null) 
            item = configProvider.getPropertySetDAO().create(piid, itemKey);
        else if (Type.getType(item.getType()) != type) 
            throw new PropertySetException("Existing key '" + itemKey + "' does not have matching type of " + type);
        ...
        switch (type) {
            ...
            case OBJECT:
                if ((value != null) && !(value instanceof Serializable)) 
                    throw new IllegalPropertyException("Cannot set " + itemKey + ". Value type " + value.getClass() + " not Serializable");
            ...
            case PROPERTIES:
                ...
                try {
                    ...
                    throw new PropertySetException(ioException.getMessage());
                }
                break;
    
            default:
                throw new PropertySetException("type " + type + " not supported");
        }
    ...
    protected Object get(Type type, String itemKey) throws PropertySetException {

        if(supportsType(type) == false)
            throw new PropertySetException("Type " + type(type) + " not supported");

        // Move this snippet into getImpl() -----------------
        HibernatePropertySetItem item = findByKey(itemKey);
        ...
        switch (type) {
            ...
            case PROPERTIES:
                ...
                try {
                    ...
                    throw new PropertySetException(saxException.getMessage());
                } catch (ParserConfigurationException parserConfigurationException) {
                    throw new PropertySetException(parserConfigurationException.getMessage());
                } catch (IOException ioException) {
                    throw new PropertySetException(ioException.getMessage());
                }
        ...
        throw new PropertySetException(buffer.toString()); 

    }


    private HibernatePropertySetItem findByKey(String itemKey) throws PropertySetException {
        return configProvider.getPropertySetDAO().findByKey(piid, itemKey);
    }


    private String encodeObject(Object object) throws PropertySetException {
    ...
    private Object decodeObject(String base64String) throws PropertySetException {
        ...
        try {
            ...
            throw new PropertySetException(classNotFoundException.getMessage());
        } catch (IOException ioException) {
            throw new PropertySetException(ioException.getMessage());
        }
